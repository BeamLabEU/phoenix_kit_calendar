...
- **Editing**: per-event authorization at modal-open (`can_edit_event?` vs the event's persisted owner) from ANY view. Creating needs exactly ONE selected calendar the viewer may edit (`can_edit_single?`); multi-selection hides New event and the save path is guarded. Read-only badge: single non-editable selection, or multi without `edit_others`.
