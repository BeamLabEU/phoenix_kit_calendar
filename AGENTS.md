...
- **Everyone view** (`?user=all`, boss's ask 2026-07-08: "who's free on Thursday"): every calendar overlaid on one month grid, each event title prefixed with its owner's short name (`Anna · Standup`), plus **person-toggle chips** to narrow the overlay to the people being compared (All/None quick buttons; `(empty)` marks users with no events; selection is LV state, resets on view switch). No single target calendar → no event creation in this mode (button hidden, save path guarded); event CLICKS authorize per event (`can_edit_event?` assigned at modal-open against the event's persisted owner), so a Boss edits inline while a Junior Manager stays read-only. Context: `list_all_events/4` with `:owner_uuids` filter, gated on `view_others`/`edit_others`.
